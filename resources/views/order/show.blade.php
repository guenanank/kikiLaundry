<div class="modal fade" data-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            {{ Form::model($order, ['route' => ['order.print_po', $order->id], 'method' => 'PATCH', 'class' => 'ajax_form']) }}
                ...
                <div class="modal-body">
                        ...
                        <div class="col-sm-offset-1 col-sm-10">
                            @if(is_null($order->dikirim))
                                <div class="form-group fg-float">
                                    <div class="fg-line">
                                        {{ Form::text('dikirim', $order->dikirim, ['class' => 'form-control fg-input date-picker']) }}
                            ...
                            @else
                                <div class="form-group has-success">
                                    <div class="fg-line">
                                        {{ Form::label('dikirim', 'Tanggal dikirim', ['class' => 'control-label']) }}
                                        {{ Form::text('_dikirim', $order->dikirim, ['class' => 'form-control', 'disabled']) }}
                                        {{ Form::hidden('dikirim', null) }}
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-condensed">
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="4" class="text-right"><strong class="text-primary">Total</strong></td>
                                    <td class="text-right"><strong class="text-primary">Rp. {{ number_format($order->jumlah_tunai) }}</strong></td>
                                    <td class="text-right"><strong class="text-primary">Rp. {{ number_format($order->jumlah_cicil) }}</strong></td>
                                </tr>
                            </tfoot>
                            <tbody>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn bgm-gray btn-sm btn-icon-text"><i class="zmdi zmdi-print"></i>&nbsp;Cetak surat jalan</button>
                        @if($order->dicetak == false && is_null($order->dikirim))
